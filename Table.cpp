...
{
	FillChar(T.X, T.Y + T.Height - 3, 1, TabChars[T.TabType][2][0]);
	FillChar(T.X + T.Width - 1, T.Y + T.Height - 3, 1, TabChars[T.TabType][2][3]);
	FillChar(T.X, T.Y + T.Height - 2, 1, TabChars[T.TabType][1][0]);
	FillChar(T.X + 1, T.Y + T.Height - 2, T.Width - 2, TabChars[T.TabType][1][1]);
	FillChar(T.X + T.Width - 1 , T.Y + T.Height - 2, 1, TabChars[T.TabType][1][0]);
	FillChar(T.X , T.Y + T.Height - 1, 1, TabChars[T.TabType][3][0]);
	FillChar(T.X + 1, T.Y + T.Height - 1, T.Width - 2, TabChars[T.TabType][3][1]);
	FillChar(T.X + T.Width - 1, T.Y + T.Height - 1, 1, TabChars[T.TabType][3][3]);
	WriteText(T.X + 2, T.Y + T.Height - 2, "Ячейка");
}
...
void EditValue(Table &T, char Ch)
{
	if (!T.ReadOnly)
	{
		...
		do
		{
			...
			Val = (TInf)strtod(S, &C);
			if ((c1 != 27) && (C[0]))
				MessageDlg("Не верный формат числа!", 0, 0x74);
		} while (C[0]);
		if (c1 != 27)
			DrawValue(T, Val);
	}
...
void InitFrame(Frame &T, int X, int Y, int Width, int Height, int Type, short Colors, char *Title)
{
	T.X = X;
	T.Y = Y;
	T.Width = Width;
